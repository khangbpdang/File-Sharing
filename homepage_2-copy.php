...
if (mysqli_connect_error()) {
	...
	$sql = "";
	if ($input !== '') {
		//$input = trim($_GET['search_input']);
		/*
		$sql = "SELECT * FROM files WHERE file_name LIKE '%$input%' OR
																			username LIKE '%$input%' OR
																			file_type LIKE '%$input%' OR
																			size LIKE '%$input%' OR
																			file_desc LIKE '%$input%' OR
																			dt_uploaded LIKE '%$input%'";*/
		//$query = explode(" ")
		$input = metaphone($input);

		$sql = "SELECT * FROM files WHERE filesound LIKE '%$input%'";
		//echo $sql;
	} else {
		$sql = "SELECT * FROM files";
	}
...
<body>
	...
	<header id="header">
		...
		<div class="search_input" id="search_input_box">
			<div class="container box_1170">
				<form action"homepage_2-copy.php" method="GET" class="d-flex justify-content-between">
