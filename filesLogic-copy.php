...
if (mysqli_connect_error()) {
  ...
  if (isset($_POST['save'])) { // if save button on the form is clicked
    ...
    if (!in_array($extension, ['txt', 'jpg', 'png', 'mp3', 'pdf', 'docx'])) {
      ...
      if (move_uploaded_file($file, $destination)) {
        $sql = "INSERT INTO files (file_hash, file_name, username, file_type, size, file_desc, dt_uploaded, downloads) VALUES ('$filehash', '$filename', '$username', '$extension', '$size', '$filedesc', CURRENT_TIMESTAMP(), 0)";
  ...
  if (isset($_GET['file_id'])) {
    ...
    if (file_exists($filepath) && (strcasecmp($username, $file['username']) == 0)) {
      ...
      $updateQuery = "UPDATE ,m files SET downloads=$newCount WHERE file_id=$id";
